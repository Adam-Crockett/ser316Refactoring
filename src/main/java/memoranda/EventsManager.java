	...
	public static Collection getRepeatableEventsForDate(CalendarDate date) {
		...
		for (int i = 0; i < reps.size(); i++) {
			...
			if (date.inPeriod(ev.getStartDate(), ev.getEndDate())) {
			  ...
			  //created method to handle the check and changes to v
			  v = dailyCheck(ev, date, v);
			  
				ev.getRepeatableEventsForDate(date, v);
			}
		}
		return v;
	}
	
	//Method created to handle some of the getRepeatableEventsForDate actions
	public static Vector dailyCheck(IEvent ev, CalendarDate date, Vector v) {
